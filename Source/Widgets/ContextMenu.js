...
/*
---
description: Right-click context menu for JFrame
provides: [ContextMenu]
requires: [More/Element.Shortcuts, Core/Element.Event, Core/Class.Extras, Behavior/Element.Data,
	...
	*/
	document.addEvents({
		...
		keyup: function(e) {
			...
			if (e.key == "esc" && ContextMenu.active) ContextMenu.active.hide();
		}.bind(this)
	});
	ContextMenu = new Class({
		...
		show: function(x, y){
			...
			//if there is a menu visible, hide it on any click
			if (ContextMenu.active && ContextMenu.active != this) ContextMenu.active.hide();
			if (this.activeMenu) {
